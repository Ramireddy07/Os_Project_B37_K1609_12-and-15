...
void findWT(int p[], int m,int bt[], int wt[], int quant){
    int rem_bt[m],i;
    for ( i = 0 ; i < m ; i++)
        rem_bt[i] =  bt[i];
    int t = 0; 
...
void RoundRobin(process_structure process[],int processes[],int size){
    ...
    for(i=0;i<size;i++){
        ...
        BurstTime[i] = process[processes[i]].bt;
    }
...
int main(){
    ...
    time = process[PriorityQueue[0]].bt;
    for(y = 0; y < PrioritySize; y++){
        for(count = y; count < PrioritySize; count++){
            if(process[PriorityQueue[count]].at < time)   {
          ...
          if(process[PriorityQueue[count]].ready == 1 && process[PriorityQueue[j]].ready == 1)

            {
...

                {
                    ...
                    if(process[PriorityQueue[count]].bt > process[PriorityQueue[j]].bt)
    ...
    printf("\nProcess[%d]:\tTime:\t%d To %d\n", process[PriorityQueue[y]].pid, time, time + process[PriorityQueue[y]].bt);

    time = time + process[PriorityQueue[y]].bt;
